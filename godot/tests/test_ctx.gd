...
## То же, но забег НЕ получает карту вылазки: раздача чистится в том же тике,
## в котором её выложили часы.
##
## ⚠️ Зачем это вообще нужно. Если игрок не выбрал карту за весь Спад,
## `RunState.auto_pick_if_needed` берёт `draft[0]` — то есть СЛУЧАЙНУЮ карту.
## Пока базовых карт было три, сьюты часов, воды, цикла и потребностей
## проходили по везению: на их сидах выпадала карта без эффектов на время и
## воду. Первая волна контента довела базовый пул до девяти, четыре из них
## правят длительность отлива или плато, — и 30 проверок в шести сьютах разом
## стали мерить не то, что написано в их именах («цикл = 3000 тиков» превратился
## в «какая карта выпала сидом»).
##
## Поэтому: тест, который меряет ЧАСЫ, ВОДУ, ЦИКЛ или ПОТРЕБНОСТИ, тикает
## отсюда. Тест, который меряет КАРТЫ, выбирает их явно (tests/test_cards.gd).
func run_ticks_no_cards(world: Object, n: int) -> void:
	for i: int in n:
		world.call("tick")
		var rs: Object = world.get("run_state")
		if rs != null:
			(rs.get("draft") as Array).clear()
		(world.get("events_out") as Array).clear()
