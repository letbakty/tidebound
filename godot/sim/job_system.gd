...
## docs/00 §6.5: «каждый свободный агент РАЗ В СЕКУНДУ оценивает доступные
## задачи». Скоринг — самое дорогое место тика (N задач × M агентов ×
## (score + длина пути)), и гонять его десять раз в секунду значит платить
## десятикратно за решение, которое спека требует принимать раз в секунду (O1).
##
## Агенты разнесены по тикам секунды через свой id: нагрузка размазана, а
## «раз в секунду на агента» соблюдается точно. Смещение детерминировано —
## два забега с одним сидом выбирают в одни и те же тики.
static func _scores_this_tick(a: SimAgent, w: SimWorld) -> bool:
	return (w.clock.total_ticks() + a.id) % Balance.TICKS_PER_SEC == 0
...
func _assign(w: SimWorld) -> void:
	for a: SimAgent in w.agents.agents:
		...
		if a.job_id != -1:
			...
			# а не идёт в пустоту. Эта проверка идёт КАЖДЫЙ тик: она про
			# корректность, а не про выбор.
