...
## Потолок надбавки от карт (`next_spring_add`) к ОДНОЙ сизигии.
##
## Надбавка копится: три базовые карты из девяти дают +1, между сизигиями
## пять раздач, и к 12-му циклу плато доходило до +5 (замер итерации 4,
## прогон C). Все склады всех профилей стоят на +2 — значит в половине
## забегов судно заставало ВСЕ склады затопленными и очки груза падали
## в ровный ноль. docs/00 §11.2 обещает игроку, что запаса в один ярус
## (склад ≥ +3) достаточно; без потолка это обещание неверно.
const SPRING_BONUS_MAX: float = 1.0
...
## Слот = один стак. 12 → 16 (итерация 4, прогон B): при 12 склады стояли
## заполненными на 87–96% у четырёх профилей из пяти, и половина всего
## простоя колонии была агентом с непустой котомкой, которому некуда её
## положить (`_start_self_haul` не находит склада со свободным слотом).
const STORAGE_SLOTS: int = 20
...
## Приток людей помогает РАСТУЩЕЙ колонии и не воскрешает умирающую: ниже
## этого числа живых новичок не приходит. Без порога механика работала ровно
## наоборот — тем сильнее, чем хуже дела, и делала вымирание недостижимым
## (docs/balance.md, итерация 1: 0 вайпов на 100 забегов).
const NEWCOMER_MIN_ALIVE: int = 3
...
##
## Нулевое деление 4 → 6 (итерация 4, прогоны D и H). Замер по фазам показал,
## что при четырёх тайлах Жадность 0 — это не «осторожно», а «стоять»:
## в Отливе, единственной фазе, ради которой сделана игра, `turtle`
## простаивал 93.6% времени, и ни одного процента из этого не приходилось
## на полную котомку. Шесть тайлов добавляют ему ровно один депозит —
## ближние руины на −2 — и удваивают добычу.
##
## ⚠️ Восемь тайлов пробовали (прогон D) и откатили: на достижимой без
## собственных лестниц части утёса самая дальняя цель отстоит от колонки
## ровно на 8, и Жадность 0 переставала запрещать хоть что-нибудь — ползунок
## становился мёртвым на нижнем делении. Деления 1–3 не трогаем: прогон D
## поднимал их до [14, 24] и не сдвинул НИ ОДНОГО числа у трёх профилей —
## там связывает достижимость, а не радиус.
const GREED_LADDER_LIMIT: Array[int] = [6, 8, 16, -1]
...
## Забег провален, если на ГРАНИЦЕ ЦИКЛА живых меньше этого числа (docs/00
## §11.2). Порог, а не ноль: колония из одного человека уже не работает —
## он не носит, не строит и не может добрать новичков (NEWCOMER_MIN_ALIVE),
## а забег без поражения — это забег без причины начать второй.
const WIPE_THRESHOLD: int = 3
