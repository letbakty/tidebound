...
extends Node
## Распознавание жестов поверх мира (этап 12).
##
...
## Псевдо-индекс «пальца» для удержания ПКМ: мышь обязана давать те же
## сигналы, что и тач, иначе жесты нечем проверить без устройства.
const MOUSE_LP_INDEX: int = -2
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventScreenTouch:
		...
		if mb.pressed and mb.button_index == MOUSE_BUTTON_WHEEL_UP:
			...
			if mb.pressed:
				_lp_index = MOUSE_LP_INDEX
				_lp_start = mb.position
				_lp_time = 0.0
			else:
				_cancel_long_press()
	elif event is InputEventMouseMotion and _lp_index == MOUSE_LP_INDEX:
		var mm := event as InputEventMouseMotion
		if _lp_start.distance_to(mm.position) > MOVE_TOLERANCE_PX:
			_cancel_long_press()
