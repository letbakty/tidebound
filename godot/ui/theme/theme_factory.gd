...
## font_scale — размер шрифта ОТДЕЛЬНО от масштаба UI (docs/03 §3.6).
static var font_scale: float = 1.0

static func size_of(base: int) -> int:
	# Кегль остаётся целым и не мельче девяти: требование Steam Deck Verified.
	return maxi(int(round(float(base) * font_scale)), 9)

static func build() -> Theme:
	var th: Theme = Theme.new()
	th.default_font = ui_font()
	th.default_font_size = size_of(UITokens.FONT_S)
...
static func focus_box() -> StyleBoxFlat:
	var sb: StyleBoxFlat = flat(Color(0, 0, 0, 0), UIPalette.accent(),
...
static func _build_button(th: Theme) -> void:
	...
	th.set_color("font_pressed_color", "Button", UIPalette.accent())
	th.set_color("font_focus_color", "Button", UITokens.INK)
	th.set_color("font_disabled_color", "Button", UITokens.FAINT)
	th.set_font_size("font_size", "Button", size_of(UITokens.FONT_S))
...
static func _build_label(th: Theme) -> void:
	...
	th.set_font_size("font_size", "Label", size_of(UITokens.FONT_S))
...
static func _build_checkbox(th: Theme) -> void:
	...
	th.set_font_size("font_size", "CheckBox", size_of(UITokens.FONT_S))
...
static func _build_lineedit(th: Theme) -> void:
	...
	th.set_color("caret_color", "LineEdit", UIPalette.accent())
	th.set_color("selection_color", "LineEdit", UITokens.ACCENT_SHADE)
	th.set_font_size("font_size", "LineEdit", size_of(UITokens.FONT_S))
...
static func _build_tabs(th: Theme) -> void:
	...
	th.set_stylebox("tab_selected", "TabContainer",
		flat(UITokens.RAISE, UIPalette.accent(), UITokens.BORDER_W, UITokens.SPACE_3))
...
static func _build_scroll(th: Theme) -> void:
	...
	th.set_stylebox("grabber_pressed", "VScrollBar", flat(UITokens.ACCENT_SHADE,
		UIPalette.accent(), 0, 0))

static func _build_progress(th: Theme) -> void:
	th.set_stylebox("background", "ProgressBar", flat(UITokens.PAPER,
		UITokens.BORDER, UITokens.BORDER_HAIR, 0))
	th.set_stylebox("fill", "ProgressBar", flat(UIPalette.water(),
		UIPalette.water(), 0, 0))
	th.set_font_size("font_size", "ProgressBar", size_of(UITokens.FONT_S))
...
static func _build_tooltip(th: Theme) -> void:
	...
	th.set_font_size("font_size", "TooltipLabel", size_of(UITokens.FONT_S))
...
static func _build_variations(th: Theme) -> void:
	...
	th.set_stylebox("panel", "PanelHud",
		flat(UIPalette.panel(), UIPalette.border(), UITokens.BORDER_W, UITokens.SPACE_1))
	...
	th.set_stylebox("normal", "ButtonPrimary",
		flat(UIPalette.accent(), UIPalette.accent(), UITokens.BORDER_W, UITokens.SPACE_3))
	th.set_stylebox("hover", "ButtonPrimary",
		flat(UIPalette.accent().lightened(0.1), UITokens.INK, UITokens.BORDER_W, UITokens.SPACE_3))
	th.set_stylebox("pressed", "ButtonPrimary",
		flat(UITokens.ACCENT_SHADE, UIPalette.accent(), UITokens.BORDER_W, UITokens.SPACE_3))
	...
	th.set_stylebox("normal", "ButtonDanger",
		flat(UIPalette.danger(), UIPalette.danger(), UITokens.BORDER_W, UITokens.SPACE_3))
	th.set_stylebox("hover", "ButtonDanger",
		flat(UIPalette.danger().lightened(0.1), UITokens.INK, UITokens.BORDER_W, UITokens.SPACE_3))
	th.set_stylebox("pressed", "ButtonDanger",
		flat(UIPalette.danger().darkened(0.2), UIPalette.danger(), UITokens.BORDER_W, UITokens.SPACE_3))
	...
	th.set_stylebox("pressed", "ButtonGhost",
		flat(UITokens.RAISE, UIPalette.accent(), UITokens.BORDER_HAIR, UITokens.SPACE_3))
	th.set_color("font_color", "ButtonGhost", UITokens.MUTED)
	th.set_color("font_hover_color", "ButtonGhost", UITokens.INK)

	th.set_font_size("font_size", "LabelTitle", size_of(UITokens.FONT_L))
	th.set_color("font_color", "LabelTitle", UITokens.INK)
	th.set_font_size("font_size", "LabelSmall", size_of(UITokens.FONT_S))
	...
	th.set_font_size("font_size", "LabelNum", size_of(UITokens.FONT_S))
