...
## Тёплый (предупреждение, жильё, очаг). Подмены в пресетах нет намеренно:
## янтарь читается всеми тремя типами дальтонизма и отличается от «опасности»
## светлотой. Но ходить за ним всё равно СЮДА: иначе следующая правка палитры
## снова разъедется по десятку файлов (аудит B4).
static func warm() -> Color:
	return map(UITokens.WARM)
