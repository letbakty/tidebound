...
const DEPOSIT_COLORS: Dictionary = {
	...
	# на глаз: lamp — сухая соляная корка, fog — холодная пресная линза,
	# bone — бледная кость отмели, rust_warm — ржавое дерево обломков.
	#
	# ⚠️ Пресная линза сперва была surf (#2d6b7a), и на кадре это оказалось
	# ошибкой: палитра холодной половины ВСЯ водяная по построению, и
	# насыщенный синий квадрат на затопленной клетке пропадал в воде. Из
	# холодных читается только светлая часть — fog. Проверено кадром в обеих
	# фазах, а не рядом хексов в редакторе.
	"brine_pool": Color("f0d6aa"),
	"fresh_seep": Color("9aabb0"),
