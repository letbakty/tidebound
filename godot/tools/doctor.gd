...
##
## ⚠️ Первой в ЛОГЕ, но не первой на экране: примерно два десятка строк про
## шрифты, ui_atlas, переводы и тему проекта движок печатает ещё в setup2(),
## до того как вообще узнает про ключ -s. Раньше них не выполняется ни одна
## наша строка, и подавить их из скрипта нельзя. Поэтому вторая половина
## лечения — текстовая: раздел про импорт объясняет ОБА рода ошибок, а отчёт
## заканчивается строкой, которая говорит то же самое последней.
...
func _initialize() -> void:
	...
	var checks: Array[Array] = [
		...
		["импорт ассетов выполнен", import_not_done(),
			...
			+ " и создаётся сама.\n        Всё красное выше этого отчёта —"
			+ " следствие того же несобранного кэша, а не поломки: сперва"
			+ " движок не находит импортированных шрифтов, атласа и переводов,"
			+ " потом не может разобрать автолоады, потому что без кэша ему"
			+ " неоткуда узнать про глобальные классы. После шага с импортом"
			+ " ни одной такой строки не останется"],
	]
	...
	# Свежий клон — единственный случай, когда красный лог выше НОРМАЛЕН, и
	# именно на нём человек решает, сломан ли проект. Последняя строка нужна
	# потому, что до отчёта он мог не долистать: ошибок там под полторы сотни.
	elif not import_not_done().is_empty():
		print("красные строки выше — от несобранного кэша, а не от поломки:"
			+ " сделайте шаг с импортом, и их не будет")
