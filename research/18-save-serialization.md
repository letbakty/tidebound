...
**⚠️ Дополнено на этапе 01 (найдено тестом, в исходный ресерч не входило): целые 64-битные значения тоже теряются.** `JSON.parse` кладёт число в `double` — мантисса 53 бита. Всё, что не влезает, тихо округляется:

```
rng.state = -5470398187538053127   →  JSON  →  -5470398187538053120
```

Состояние `RandomNumberGenerator.state` — ровно такое число. После загрузки генератор пошёл бы другой последовательностью, и тест «продолжение после save/load совпадает с непрерывным прогоном» падает, хотя `int()` в `from_dict` написан правильно.

**Правило: любое поле, способное выйти за ±2^53, сериализуется СТРОКОЙ** (`str(v)` / `String.to_int()`). Сегодня это `rng.seed` и `rng.state` (уже сделано в `sim/sim_rng.gd`); при добавлении новых полей — проверять. Обычные счётчики (тики, id, количества) в 2^53 укладываются с запасом.
...
**⚠️ Дополнено на этапе 05 (найдено тестом): `full_precision = true` тоже НЕ гарантирует round-trip.** `JSON.stringify` печатает double 16–17 значащими цифрами, и для произвольного double этого не хватает: накопленная позиция агента `0.9999999999999969` читается обратно как `...68`. Хеш совпадает, а продолжение симуляции после загрузки расходится с непрерывным прогоном — то есть падает главная приёмка этапа 11.

**Правило: каждая дробная величина СОСТОЯНИЯ квантуется в момент изменения**, а не при сериализации (`Balance.quant`, шаг 1e-4). Тогда в сейв попадает короткое представление, которое читается обратно точно, и живой мир с загруженным остаются идентичны. Сегодня квантуются позиция агента (`x`), доля подъёма по лестнице (`climb_t`) и уровень воды.

⚠️ И отдельная ловушка внутри ловушки: квантовать надо **делением целого** (`float(roundi(v * 10000.0)) / 10000.0`), а НЕ через `snappedf(v, 0.0001)`. `snappedf` умножает на 0.0001 — число, само по себе не представимое точно, — и выдаёт `3.8000000000000003` вместо канонического `3.8`, то есть ровно ту величину, которая round-trip не переживает.
